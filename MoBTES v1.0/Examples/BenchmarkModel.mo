...
model BenchmarkModel "MoBTES model for comparison to 3D FEM models"
	...
	Components.SeasonalThermalStorages.BTES Storage annotation(Placement(transformation(
		origin={20,-20},
		extent={{-20,-20},{20,20}})));
	equation
		 connect(FLOW.flowPort, volumeFlow1.flowPort_a) annotation(
		   Line(points = {{0, 75}, {5, 75}, {10, 75}, {10, 60}, {10, 55}}, color = {255, 0, 0}, thickness = 0.0625));
		 connect(realExpression1.y, volumeFlow1.volumeFlow) annotation(
		   Line(points = {{-64, 45}, {-59, 45}, {-5, 45}, {0, 45}}, color = {0, 0, 127}, thickness = 0.0625));
		 connect(Tin.y, RETURN.ambientTemperature) annotation(
		   Line(points = {{-64, 90}, {-59, 90}, {60, 90}, {60, 69}, {55, 69}}, color = {0, 0, 127}, thickness = 0.0625));
		 connect(Tin.y, FLOW.ambientTemperature) annotation(
		   Line(points = {{-64, 90}, {-59, 90}, {-25, 90}, {-25, 69}, {-20, 69}}, color = {0, 0, 127}, thickness = 0.0625));
		connect(Storage.BTESflow,volumeFlow1.flowPort_b) annotation(Line(
			points={{10,0},{10,5},{10,30},{10,35}},
			color={255,0,0}));
		connect(Storage.BTESreturn,RETURN.flowPort) annotation(Line(
			points={{30,0},{30,5},{30,75},{35,75}},
			color={255,0,0}));
	annotation(
